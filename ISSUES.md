...
2.) **Chrome entailment** — *slice 1 done* (2026-06-29): the create surface now reads the parent's `childrenSpec` (`childrenPolicy.ts` → `allowedChildKinds`/`canHaveChildren`; `registry.ts` → `reRootCreateKindsFor`), so the node-create picker offers only admitted kinds (`job`→node/job, not org) and a content-free lens (`jobs`) offers no "Add"; the lens-aware shell suppresses the DataCard/chevron for content-free kinds (IMPLEMENTATION.md → *#5 slice 1*). Remaining: **decouple placement's render-location from navigability** so a `jobs` node can show each `job` as an inline Data-Card row that is still re-rootable (layout driven by `childrenSpec`) — and navigable rollup rows / rich lens rows (currently `KindAdornment` lists job names as dead text); blocked on `jobs`-as-container (#6c/logbook). Plus the **value-shape vocabulary** (`scalar | block | stream | composite` driving layout, retiring `hideLabel`/`blockValueLayout`) and the fuller manifest-driven shell regions (meta-fields→Details/Settings, grouping-tag→section).
