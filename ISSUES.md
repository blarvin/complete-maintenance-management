...
2.) **Chrome entailment** — *now unblocked* (#6b built four re-root shells). Factor hardcoded shell drawing into a renderer reading the manifest (`re-root`→Up, `open`→Add, meta-fields→Details/Settings, grouping-tag→section). First concrete slice: **decouple placement's render-location from navigability** so a `jobs` node can show each `job` as an inline Data-Card row that is still re-rootable (layout driven by the container's `childrenSpec`); and restrict/hide the create surface by `childrenSpec` (a content-free lens offers no "Add" — currently a Jobs view wrongly offers Node/Org).
