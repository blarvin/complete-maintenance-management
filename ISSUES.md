...
6.) On fieldDefinitionId — you've got a real smell, but I'd not pull that thread in §6b. It's load-bearing today, not vestigial: it's how a field instance finds its config (which now lives as sub-field children on the Definition), and it doubles as the version pointer for edit-is-fork (#7). The principled retirement isn't "delete the column" — it's "model the instance→Definition link as an internal, revision-pinned Edge," i.e. the same Edges machinery asset-doc begins in this pass and #6c/#7 finish. So my vote: keep the column now, make the create-time check placement-driven (inline kinds require it, re-root kinds don't), and log "retire fieldDefinitionId → Edge" as the real future item. Retiring it now turns a stub pass into an Edges-plus-versioning migration and front-runs two later clusters. **Update (2026-07-01):** `logbook` **landed (#6c, minimal) with no authored-in config** — it's the first *policy-container* re-root kind that will actually *want* a bound Definition (entry label, staleness policy), so it's the concrete forcing case for the placement-driven create-check + the eventual `fieldDefinitionId → definitionId` rename and decoupling the Definition contract from `placement: inline`. Deferred with this item / #7.
...
10.) **Manifest→render-mode derivation is duplicated across consumers** — the same "plain container vs lens (Provision + typed Derivation) vs untyped-rollup (Derivation, no Provision)" pattern-match is re-derived ad hoc in three places with three shapes: `TreeNodeDisplay.tsx` (`manifest.provision ? derivation?.targetKind : undefined` → `isLens`), `BranchView.tsx` (same, for the re-rooted gather), and `KindAdornment.tsx` (the inverse, `!derivation || provision`). This is a real axis of node state (which mode → what renders + where children come from) living outside the FSM as loose booleans. Collapse into one registry-side selector — `nodeRenderMode(kind): 'plain' | 'lens' | 'derivation-chip'` (or a thin discriminated union carrying `targetKind`) — so all three read one source of truth. Same seam shape as `isReRoot`/`isInline` in `placement.ts`, just extended to the derivation/provision combination. Pure consolidation, no behaviour change; the core appState FSM is unaffected.

11.) **Provisioned-lens lifecycle (jobs + logbook)** — provisioning is create-time only (`ensureProvisionedLenses` on `CREATE_ELEMENT`), so three gaps remain, now generic across all `PROVISIONED_LENSES` (one fix covers both lenses): (a) **backfill** — nodes created before a lens kind existed carry no lens (today: reseed/wipe at prototype scale); (b) **de-provision/GC** when the last entry below is removed; (c) **hide an empty lens**. The canonical upward ancestor-walk provisioning (ELEMENT-MODEL §lens) is superseded by the per-node v1 — revisit only if the per-node rollup proves insufficient. *(Moved from LATER — a leftover of in-flight lens work, not a whole deferred feature.)*

12.) `capabilityEngine` ****`ancestors`**/**`edges` **traversal** — only `children` (direct/transitive) is built; `ancestors` (inheritance, feeds the cascade #4) and `edges` (curated membership, the Edges family #3) currently *throw* in `capabilityEngine.ts`.

13.) `asset-doc` **real target picker + editing** — the target is a raw element-id paste with no config; wants a picker constrained by an allowed-target-kind config, plus editing a saved link.

14.) **Field-composer restriction by** `childrenSpec` — the re-root create surface restricts by the parent's allowlist, but the field composer still offers all `FIELD_KINDS`. No-op today (every container admits every field kind); wire `allowedChildKinds ∩ FIELD_KINDS` if a kind ever narrows admitted fields.

15.) `job` **admits** `job` **children (sub-tasks)?** — `job.allowedKinds` still includes `job`, but no picker mints a sub-job (trimmed by `isLensSurfaced`) and a `job` gets no `::jobs` lens. Decide nested-jobs vs job-subtypes (Task/Work-Order/Project) — a `capabilities.ts` allowlist call, coupled to the job-subtype question (#3).

16.) `node.allowedKinds` **real allow-policy** — currently a provisional literal (`['node','text-kv',…]` + `TODO(#6)`) dodging a `registry`→`capabilities` cycle; derive the honest "child nodes + field kinds" policy.

17.) **Derive the** `Value` **union from the registry** — `Kind` is registry-derived but `DataFieldValue` is still hand-declared; needs a type-level kind→value map (`KindValueMap`), exhaustiveness-checked so a missing kind→value entry is a compile error. Rides the config/value rework.

18.) **Enforce manifest key === manifest** `kind` — nothing checks a manifest registered under `'text-kv'` actually declares `kind: 'text-kv'`; a typed-key helper would make a mismatch a compile error. Low value until kinds multiply.

19.) **Per-kind** `coherence` **overrides** — the `coherence?(caps)` hook on `ManifestIdentity` is available but unused (global rules live in `checkCoherence`); add per-kind rules only when a kind needs one beyond the global set.
