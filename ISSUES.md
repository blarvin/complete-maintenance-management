...
1.) **Decide the fate of `job` children materialized under a node** — a `job` created under a tree node persists as that node's child *and* surfaces in the node's `Jobs` lens rollup, so it appears twice; letting jobs pile up as materialized siblings alongside the rollup looks wrong. Decide the model: hide `job`-kind children from the normal Data-Card/child list (they "live in" the `Jobs` node), or let them visibly migrate into the `Jobs` node at creation — which likely wants an animation or an indicator affordance so the move isn't jarring. Couples to the jobs-as-container decision (#6c / code-work-map §5).
