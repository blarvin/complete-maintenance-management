...
description: The tree-native Add Surface landed 2026-08-15; the composer and legacy add-field surfaces are dormant, not deleted, and [Fields UI]-tagged issues describe them
metadata: 
  node_type: memory
  type: project
...
deleting them is a decision the user makes, not a mop-up (docs/ISSUES.md →
Tech Debt, *The composer stack is dormant*). `pendingMode` is **not** in that
set any more: the Add Surface's value slot is the kind's real Renderer in
`pendingMode`, so it is load-bearing again.
...
**How to apply:** `[Fields UI]` in ISSUES.md no longer means "parked until the
new UI lands" — it now means "describes a surface nothing mounts", and whether
such an item is fixed or deleted follows the Tech Debt item above. Don't tag new
Add Surface work with it. The contract for the surface is
