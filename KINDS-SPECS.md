...
| **Job**                    | node behavior              | `node`               | `re-root`            | confirms | `placement = re-root` (navigate *into* it); its synthetic *Jobs* container is a rollup — same `SourceSpec` as Logbook                              |
| **Person**                 | node behavior              | `node`               | `re-root`            | confirms | the canonical cross-`Element` reference target; trait-degenerate (traits = base node) — a distinct `Kind` earned by *identity*, not trait         |
