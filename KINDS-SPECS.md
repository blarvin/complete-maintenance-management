...
| **Logbook + Log Entry**    | node behavior + entry kind | `tree` / `composite` | `re-root` / `inline` | extends  | generalize registry key to full `Kind`; append-only children; collection sub-fields (Finding G)                                                   |
| **Job**                    | node behavior              | `tree`               | `re-root`            | confirms | `placement = re-root` (navigate *into* it); its synthetic *Jobs* container is a rollup — same `SourceSpec` as Logbook                             |
| **Person**                 | node behavior              | `tree`               | `re-root`            | confirms | the canonical cross-`Element` reference target; trait-degenerate (traits = base node) — a distinct `Kind` earned by *identity*, not trait         |
...
2. **Node behaviors are first-class kinds, but all share `nature: 'tree'`.** Logbook / Job / Person are promoted to `Kind` values (`kind: 'logbook' | 'job' | 'person'`) registered in the *generalized* registry alongside fields [???]. They are **not** distinct natures — every one is `nature: 'tree'` (recursive + navigable, framework-owned). The nature is deliberately named `tree`, **not** `node`, so a kind name never doubles as a nature name: "node" means exactly one thing — the base kind and its surface — and per SPEC, *Nodes are the Tree*. What differs between behaviors is a small bundle of **traits** (see *Node behaviors as trait bundles* below). The registry **key** widens to the full `Kind` union so behaviors can register; `node` stays framework-privileged.
...
The "double duty" worry — that one word would name both the privileged framework kind and the nature of behaviors like Job — is resolved twice over. By **naming**: the nature is `tree`, not `node`, so the nature value and the `node` kind never collide. And by **structure**: **the plain Asset node is the degenerate behavior**. Every tree-natured kind shares exactly two things: it is *recursive* and *navigable*. That is `nature: 'tree'`, owned by the framework. Where the behaviors differ is a `tree` trait descriptor:


| Trait                                  | Asset node | Logbook                                 | Job                                          | Person   |
| -------------------------------------- | ---------- | --------------------------------------- | -------------------------------------------- | -------- |
| **childKinds** — what may be a child   | `*`        | `log-entry` only                        | job-related / `*`                            | `*`      |
...
```ts
// PROVISIONAL — the `tree` descriptor slot
type TreeBehaviorSpec = {
...
- **Person is the trait-degenerate behavior:** its traits are *identical* to the base Asset node (`childKinds: '*'`, physical, no aggregation). A Person has ordinary child Nodes (e.g. a *Qualifications* node holding individual *Qualification* children) alongside its Data Card of Fields (name, role, contact). So Person earns a distinct `Kind` not by trait displacement but by **identity** — it is the nameable, semantically-typed reference target (`allowedKinds: ['person']`) and picker entry. That is a legitimate (and worth-stating) finding: the registry distinction between "base node" and "a named node kind" can be *semantic* rather than structural. Consequence: the `childKinds: 'none'` leaf policy is now **unexercised** by the eleven — it stays a supported value awaiting a future leaf kind (e.g. *Qualification*), not Person.
...
type KindManifest = {
  ...
  nature: 'stored' | 'composite' | 'derived' | 'reference' | 'tree';
  ...
  tree?: TreeBehaviorSpec;          // nature: 'tree' (behaviors; base node = degenerate)
...
| G   | A composite (`log-entry`) needs *collection* sub-fields (0..n tags / @-mentions) alongside *singletons* (one body)                                                                                                                                                                                                                                                                                                                                                                                | `SubFieldSpec` gains a **cardinality** (`one`                                                                                                                                                                                                                                                                                                                                     |
| H   | A reference may pin a **specific history revision** of its target (`approval` → `${targetId}:${rev}`), not just the live element. Its **validity is a function of pinned-rev vs current-rev** — the approval goes *stale* when the target advances past the approved revision.                                                                                                                                                                                                                    | `TargetSpec` gains `pin: 'live'                                                                                                                                                                                                                                                                                                                                                   |
| I   | The `virtualParents` overlay (a first-class model property per SPEC: `{ parentId, siblingOrder, navMode, render }`, portal vs citation) must reduce onto `nature: 'reference'` with **no new primitive** — confirming the SPEC claim that "`virtualParents`, the cross-linked field, and the in-link all reduce to one edge." **Both authoring UX** — *adopt-here* (from the referrer's card) and *appears-also-under* (from the target element) — must be expressible without forking the model. | *Confirmation, not extension.* `other-end` is the reference whose edge **is** a `virtualParents` entry. `render: 'portal'` → navigable tree appearance (acts like `re-root` at the other end); `render: 'citation'` → non-navigable inline row (like `asset-doc`). Both authoring directions write the **same single edge**, distinguished only by `target.authorFrom: 'referrer' |


---

## Open tensions (to resolve before lock)

Three points where the API as drafted holds, but on heavy lifting — each worth a deliberate decision before the lock criterion is declared met.

1. **Composite lifecycle is split between an atomic core and an open tail.** Locked-decision #1 mints a composite in one transaction; but `cardinality: 'many'` sub-fields "start empty and the user adds/removes instances at runtime." So a composite's identity spans an *atomic core* (the `one` children, minted together) plus an *open-ended tail* (the `many` slots, grown later). That is two lifecycle regimes under one "composite" word. The guard to hold explicitly: the `many` tail is **kind-constrained** (only the declared `kind` may be added) and so is *not* the open, user-grown card a node has — otherwise "composite" quietly collapses back into "node." Decide and state the boundary.
2. `**other-end` is called pure confirmation, but its value shape is the odd one out.** Finding I claims `other-end` reduces to `nature: 'reference'` with **no new primitive**. Yet its value is "a `virtualParents` entry" — structurally unlike the other three reference value shapes, and its **source of truth lives on the target element**, not on the edge-holding referrer. That target-owned asymmetry is real. The reduction likely still holds (the overlay *is* the canonical edge; there is genuinely no copy), but "no new primitive" is load-bearing and should be stress-tested — confirm the resolver/sync path treats a target-owned edge identically to a referrer-owned one before calling Finding I locked.
3. **Logbook's own entries vs. its descendant rollup is the one unresolved *data-model* question.** Reusing the aggregator `SourceSpec` for the rollup is clean and confirmed. What's open (flagged in *Node behaviors as trait bundles* and again in the `logbook` stub) is whether the Logbook card shows its **own direct entries**, a **rollup of descendant entries**, or **both** — and if both, how an entry authored locally still surfaces on every ancestor Logbook without losing where it was written. Unlike most open questions in this doc, this one changes the **data model** (where entries are parented and how rollup de-duplicates against local), not just the render.
...
Generalize today's `Record<ComponentType, KindManifest>` to the full `Kind` union. `node` is registered but **framework-privileged**: its recursion/navigation live in the framework (TreeNode), and its manifest carries only the base `TreeBehaviorSpec` (`childKinds: '*'`, physical, no aggregation). Behavior kinds (`logbook`/`job`/`person`) register beside it.
...
| `tree`      | none on the node itself; recursion is framework                                                     | `name`/`subtitle`/structural props LWW'd as today; children are independent Elements                                                                                             | node, logbook, job, person                         |
...
// tree — node behaviors (sketched earlier)
type TreeBehaviorSpec = {
...
type KindManifest = {
  ...
  tree?: TreeBehaviorSpec;        // tree (node behaviors)
...
`nature: tree`, `container: logical` (a logical grouping, not a physical part — perfectly valid). A Job is a sub-asset-like Node attachable under whatever Node needs the work, and like any Node it carries an open, user-grown Field set: due date, costs, materials, skills, risk assessment, scope choices, work descriptions, goals, plus child sub-jobs. The canonical contrast: Job is a *node behavior you navigate into*, vs. Equipment Plate's *inline composite field*.

**Assignee is not part of the Job kind.** A person assigned to a job is an ordinary `reference` Field on the Job's card (`target.allowedKinds: ['person']`) — the same in-link machinery as `asset-doc`, just constrained to `person` targets. The Job *manifest* hardcodes no assignee slot; doing so would betray the open-field-set rule as much as "nodes shall have fields" would. Person enters the system as a *reference target*, not as a property of Job.

**The Jobs container is a rollup, not a parent.** Jobs are canonically parented under the node needing work; `autoProvision: synthetic-container` mints a per-asset **Jobs container** Node whose `aggregates` (`scope: subtree`, `matchKinds: ['job']`) surfaces every descendant job at the asset root — *identical machinery to the Logbook rollup*, over navigable `job` nodes instead of inline `log-entry` composites. So `autoProvision` and `aggregates` are the same concept wearing two hats; Job's own `aggregates` stays `no` — the aggregation lives on the synthetic container, not on the job.

### `person` (node behavior)

`nature: tree`, `childKinds: '*'` — **not a leaf** (changed). A Person has ordinary child Nodes (e.g. a *Qualifications* node holding individual *Qualification* children) alongside its Data Card of Fields (name, role, contact). Its traits are therefore *identical to the base Asset node*: Person is the **trait-degenerate behavior**, earning a distinct `Kind` by **identity** — it is the canonical, nameable cross-`Element` reference target (assignee on a Job, author of a log entry; `allowedKinds: ['person']`) — not by any trait displacement. This reframes what Person validates: not leaf-ness (now unexercised by the eleven — `childKinds: 'none'` remains a supported policy awaiting a future leaf kind such as *Qualification*), but that **a kind may be structurally identical to base node yet still warrant registration for semantic/reference reasons.**
