...
***Intent:*** *the `Element` model makes new surfaces cheap; this document is where we design the plugin seam that keeps them honest — one manifest per kind, against one registry, proven by nine concrete kinds.*
...
The nine kinds below are not a grab-bag. Each one forces **one specific seam** that today's flat `KindManifest` (`src/kinds/`) lacks. If the registry/manifest API designed here absorbs all nine **without special-casing**, the API is proven. That is the lock criterion for this document.

### Seam map

| Kind | Category | `nature` | `placement` | The seam it forces |
| --- | --- | --- | --- | --- |
| **Single Image + Caption** | composite | `composite` | `inline` | sub-field seam, minimal (2 children); retires the single-image latent-composite debt |
| **Equipment Plate** | composite | `composite` | `inline` | sub-field seam at scale (grid of `Label : Value` children) |
| **Asset Gallery** | aggregator | `derived` | `inline` | computed value via descendant traversal; nothing stored → sync must not LWW it |
| **Asset Doc** | in-link | `reference` | `inline` | internal reference (target `Element` id); renders live content; edge is LWW'd, target is not |
| **Part Supplier Link** | out-link | `reference` | `inline` | external reference (URL); "open out" behavior; no live fetch |
| **Key-Value + Chart** | stored, graphical | `stored` | `inline` | renderer reads the **history stream**, not just the current value; heavy → lazy |
| **Logbook + Log Entry** | node behavior + entry kind | `node` / `stored` | `re-root` / `inline` | generalize registry key to full `Kind`; append-only children |
| **Job** | node behavior | `node` | `re-root` | `placement = re-root` (navigate *into* it) |
| **Person** | node behavior | `node` | `re-root` | cross-`Element` reference target (assignee, log-entry author) |
...
3. **Drafting order:** probe-kinds first (this draft), then extract and lock the API, then fill the remaining seven.
...
| G | A composite (`log-entry`) needs *collection* sub-fields (0..n tags / @-mentions) alongside *singletons* (one body) | `SubFieldSpec` gains a **cardinality** (`one` \| `many`); `composition` is no longer a flat fixed list of singletons. Refines Finding C |
...
export const KIND_REGISTRY = {
  ...
  'asset-doc': …, 'part-supplier-link': …,                          // reference
...
| `reference` | an *edge*: internal Element id or external URL | **LWW the edge only**; the target's content is owned by the target and never touched | asset-doc, part-supplier-link |
...
// reference
type TargetSpec = {
  scope: 'internal' | 'external';     // value is { targetId: ID } | { url: string }
