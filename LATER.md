...
- **`job` admits `job` children (sub-tasks)** — `job`'s `childrenSpec.allowedKinds` includes `job`, so the create surface under a job offers "Job" (and a job gets its own provisioned `jobs` lens). Flagged dubious (2026-06-29); whether nested jobs are the right model (vs. job-subtypes Task/Work-Order/Project) is a `capabilities.ts` allowlist decision, deferred with the job-subtype question (#6c).
