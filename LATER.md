...
- **Rich + navigable lens rows** — the Jobs rollup (`KindAdornment`, isParent) lists job *names* as dead text. Both (a) per-job props inline (priority colour, owner, status) and (b) making each row navigate/re-root into its `job` are deferred — (b) is the same inline-yet-navigable decoupling as the `jobs`-as-container bullet above. (A `job` is still navigable/expandable from the normal tree today; only the rollup entries are inert.)
