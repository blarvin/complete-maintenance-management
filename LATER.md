...
- **Derive the `Value` union from the registry** — `Kind` is now `keyof typeof KIND_REGISTRY` (+ `"node"`), but `DataFieldValue` is still hand-declared: the runtime registry holds components/functions, not value *types*, so true derivation needs a separate type-level kind→value map (`KindValueMap`), exhaustiveness-checked so a missing kind→value entry is a compile error. Deferred into the config/value rework (Migration #2/#3), where value shapes are reworked anyway.
- **Enforce manifest key === manifest `kind` field** — Nothing currently checks that a manifest registered under key `'text-kv'` actually declares `kind: 'text-kv'` (the pre-rename `satisfies Record<ComponentType,…>` never did either). A typed-key helper could make a mismatch a compile error; low value until kinds multiply (Migration #2).
...
- **Disposition honoring (cascade arbiter)** — `owned`/`delegated`/`pinned` is **encoded** on each `ConfigSubField` but nothing acts on it; everything reads live from the Definition. Copy-at-mint for `owned` and override-disable for `pinned` are the cascade arbiter's job (ISSUES Architecture Migration #4). Until then, edit-is-fork (new `fieldDefinitionId`) already prevents a Definition change from rewriting existing instances.
- **Per-sub-field reactive signals** — renderers assemble the whole config object on read via `getFieldDefinitionById` (a `useResource$` keyed on `fieldDefinitionId`). Live propagation of an individual Definition sub-field edit into mounted instances is unneeded in Phase 1 (no Definition-edit UI; fork-not-mutate). Revisit if/when Definitions become live-editable.
...
- **`jobs` lens lifecycle** — de-provision/GC when the last `job` below is removed; hide an empty lens; back-fill a lens onto nodes created *before* provisioning existed (today only new container re-root creates get one). The canonical **upward ancestor-walk provisioning** (ELEMENT-MODEL §lens) is also superseded by the per-node v1 — revisit only if the per-node rollup proves insufficient. Also deferred: a **collapsed Jobs-row count badge** — the rollup count now lives inside the expanded card, and `KindAdornment` no longer chips the lens (it keeps only `org`'s descendant count).
...
- **Rich lens rows (the "primary line")** — **(b) navigable/expandable rows ✅ done (2026-06-29, #5 slice 2):** the Jobs rollup renders each gathered child as a generic `NavigableRow` (name re-roots via `navigateToNode$`, chevron expands a `FieldList` peek with `hideAddSurfaces`), retiring the dead-text list (IMPLEMENTATION.md → *#5 slice 2*). **(a) rich per-job props inline** (priority colour, owner, status — the "primary line") stays deferred, likely answered with `Action`/lifecycle. Also deferred: an optional `onActivate$` prop on `NavigableRow` for a future non-navigable consumer (today it reads `navigateToNode$` from context, correct for every re-root target).
...
- **Field-composer restriction by `childrenSpec`** — the create surface now restricts *re-root* kinds by the parent's allowlist, but the field composer still offers all `FIELD_KINDS` regardless of parent. A no-op today (every container kind admits every field kind), so left unbuilt; wire it the same way (`allowedChildKinds ∩ FIELD_KINDS`) if a kind ever narrows its admitted fields.
- **`job` admits `job` children (sub-tasks)** — `job`'s `childrenSpec.allowedKinds` still includes `job`, but as of the #5 container half a `job` **no longer gets its own provisioned `jobs` lens** (`ensureJobsLens` skips lens-surfaced kinds) and no create surface mints a sub-job directly under a job (`job` is trimmed from every picker by `isLensSurfaced`). Whether nested jobs are the right model (vs. job-subtypes Task/Work-Order/Project) remains a `capabilities.ts` allowlist decision, deferred with the job-subtype question (#6c).
