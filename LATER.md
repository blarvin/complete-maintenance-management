...
- **Rich + navigable lens rows** — the Jobs rollup (`KindAdornment`, isParent) lists job *names* as dead text. **(b) navigable/expandable rows is promoted to #5 slice 2** (jobs-as-navigable-field-rows — name re-roots, chevron expands the job's `FieldList`; code-work-map §5). **(a) rich per-job props inline** (priority colour, owner, status) stays deferred — the "primary line" question, likely answered with `Action`/lifecycle. (A `job` is already navigable/expandable from the normal tree; slice 2 brings that to the rollup rows.)
