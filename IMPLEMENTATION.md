...
- **Still deferred for #5:** the value-shape vocabulary (`scalar | block | stream | composite`) and the formal render-location axis driven by container `childrenSpec` (the *container* half — inline Data-Card-row jobs that are still re-rootable); the row's *primary* line (status/lifecycle, waits on `Action`).
