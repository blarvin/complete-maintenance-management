...
**Two handlers do more than delegate.** `CREATE_ELEMENT` calls `ensureProvisionedLenses` after the write — that is where a new node gets its Jobs and Logbook containers. `CREATE_ELEMENT_FROM_DEFINITION` resolves the Definition first and **snapshots `def.label` into the element's `name`**, so an instance keeps the label it was minted with and renaming a Definition never rewrites existing instances.

*(A 2026-08-17 amendment here declared the snapshot's premise gone, for the edit-in-place design; that design was rejected 2026-08-20 and the snapshot serves fork-not-mutate as originally written.)*
...
**The seeder kept the writing and lost the content.** The seeder (then `seedDefinitions.ts`, since reworked — see *Per-population bootstrap* below) loops `packDefinitions()`; the chrome rows stay in code, because chrome is app structure — a pack that could remove the Library root could brick the app. `SEED_VERSION` went to 10 only in the commit that changed row content; the mechanical move shipped byte-identical data deliberately, so it could not force a reseed. Its test counts against the pack instead of a copied label list, which is what stops the SPEC-vs-seeds drift from re-forming inside the tests.
...
### Per-population bootstrap (2026-08-22)

`seedDefinitions.ts` became `src/data/services/bootstrap.ts`. The seeder wrote two unrelated batches — the Library chrome trio and the pack's 30 Definitions — behind **one** `SEED_VERSION` key, so re-authoring the pack also rewrote the chrome, and a third batch could not join without dragging both along. A **population** is now the unit: `{ id, revision, mode, rows, retiredIds? }`, gated on its own `seeded:<id>` syncMetadata key.

**`rows` is a thunk, not an array.** Pack resolution must not happen at module-import time — the same constraint that made the lens schedule a function (above). A population declared as data with an eager `rows` array would re-introduce exactly the import-time evaluation the pack seam removed.

**`mode` exists before its first user, deliberately.** `upsert` overwrites on a re-run; `ensure` is put-if-absent. Every population today is `upsert`, which is correct while nothing user-edits these rows — but `ensure` is what a seeded *user-editable* tree needs (LATER → *typed trees* → the four config trees), and adding it later means reopening the runner and re-deciding the transaction shape. It is built and tested; switching a population over is then a one-word edit at the moment that population becomes editable.

**`retiredIds` is the half an upsert-only seeder structurally cannot have.** Upsert writes what the pack ships and is silent about what it stopped shipping, so a renamed row lives forever: a dev profile still carried `library_root` from before the `lib_root` rename, passing `isDefinitionRow` and listing as a 31st Definition in a 30-row pack. The prune runs **inside the revision-gated pass**, not on every boot — retirement is part of a specific revision's migration, and a boot already at that revision has nothing to migrate. The practical consequence when hand-testing: you get one observation of a prune per revision bump, and re-adding the row afterwards will not re-trigger it.

**Legacy-key migration is a delete, not a translation.** `runBootstrap()` drops `definitionsSeededVersion` if it finds it. It does not map the old version onto the new revisions — the new keys are absent on such a profile anyway, so both populations run their first pass. That one-time double-write is harmless (upsert, byte-identical rows) and is precisely what carries the retired-id prune onto old profiles.

**The lens re-stamp writes `db.elements` directly.** `restampUnboundLenses` (`provisionLenses.ts`) repairs a lens that minted with `definitionId: null` because its policy Definition was unresolvable at the time. It bypasses the adapter for two reasons: `StorageElementUpdate` deliberately carries **no** `definitionId` — a binding is fixed at mint as far as user-facing edits are concerned, and this is not one — and the stamp is deterministic per client, since every client resolves the same id from the same pack. Enqueueing it would be N identical sync ops for N clients, the same argument the bootstrap runner writes under; the remote mirror catches up as each client boots.

**Its `treeType === 'business'` guard is load-bearing, not defensive.** A policy Definition is itself a row of a lens kind — `fd_logbook_policy` is kind `logbook`, unbound, undeleted, sitting in the `library` tree. Without the tree filter it satisfies every other condition and gets stamped with its own id. Pinned by `provisionBackfill.test.ts`.

**Boot state split from `initialized`.** The catch in `doInitializeStorage` set `initialized = true` after a boot that got no further than `db.open()`, so a failed init was indistinguishable from a clean one while the app ran with no Library and every write throwing. `initialized` still goes true (the app should render rather than show nothing) but a separate `degraded` flag records the throw, and `getBootState()` → `pending | ok | degraded` is what the UI reads. `App` raises a persistent error toast with a Reload action on `degraded`. Create surfaces are deliberately **not** gated on it — that is a separate open item.

**A persistent toast is `PERSISTENT_DURATION`, not `Infinity`.** `setTimeout` clamps to a 32-bit delay and fires *immediately* on overflow, so `Infinity` would make "persistent" the one value that dismisses fastest. It is `2_147_483_647` (~24 days), which outlives any session.
