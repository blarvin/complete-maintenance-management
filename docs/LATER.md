...
- **A general pack format** — `FieldDefinitionPack` is shaped around the one thing it carries. Whether packs are the general form for *any* shipped population (config-tree skeletons, a Templates bundle) is a real question, but generalising on a single instance would be guesswork: **wait for the second concrete pack** and let the two of them show where the seam is. Sketched 2026-08-21 alongside the per-population bootstrap, which took the same view — `BootstrapPopulation` describes populations, not pack format.
- **Org / user layers, per-node override, source chips** — need the arbiter (ISSUES → Architecture → *The cascade / arbiter*).

**Constraints found while scoping (2026-08-14):**

- ~~**Additive (put-if-absent), not upsert.**~~ **Mechanism landed 2026-08-22.** Upsert contradicts forked-never-mutated (SPECIFICATION.md §611) the moment Definitions become user-editable *or* packs become swappable: reloading a pack would rewrite a Definition that live instances are bound to. Each bootstrap population now carries a `mode` (`upsert` | `ensure`), so the choice is per-population data rather than a property of the seeder. Every population is still `upsert`, which is right while nothing user-edits them; what remains is **switching a population to `ensure` on the day its rows become user-edited**, which is a decision at that moment, not scheduled work.
...
- **Boot ordering.** `provisionPolicy.ts` must stay component-free and sync-readable (`provisionLenses.ts` imports it on the `CREATE_ELEMENT` path and cannot pull the registry), so a fetched pack must be fully loaded before the first mint. `initStorage.ts` currently seeds *after* the command bus initialises.
- **A failed fetch must not degrade silently.** `ensureProvisionedLenses` stamps lens policy only if resolvable; with a fetched pack, that guard becomes the path a fetch failure takes, minting unbound lenses. **The permanence of that is fixed** (2026-08-22): `restampUnboundLenses` repairs an unbound lens on the next boot, a failed init is distinguishable (`getBootState()` → `degraded`, surfaced as a toast), and construction defaults that don't resolve are reported rather than dropped. So the degradation is transient now instead of permanent — but *transient* still assumes a later boot where the fetch succeeds, which is exactly what a bundled fallback guarantees and a fetch-only pack does not. Bundled fallback; never boot packless.
...
- **The `config` tree** — org/role/user prefs as `config`-tree Elements + the arbiter that reads them (app→org→role→user cascade). Its first two concrete tenants are the construction-default and lens-policy bindings — see Data Model → *Definition Packs* and UI/UX → *Config-tree UI*.
  ...
  **Seeded roots, provisioned subtrees** (sketched 2026-08-21 while building the per-population bootstrap). The four config trees split cleanly along an axis that already exists in the code, so neither half needs new machinery: the **skeleton roots** are genuine boot-time singletons with fixed ids — a bootstrap *population*, exactly what `services/bootstrap.ts` writes. The **per-person subtrees** (a colleague's profile, their settings) are not singletons and must not carry authored ids: they are *provisioned* at `${userId}::settings`, the existing `ProvisionSpec` id scheme with a trigger other than node-create. The rule that falls out: **fixed ids only for genuine singletons; per-entity ids derive from their owner.** Getting this backwards — seeding a row per user — is what would force the seeder to know the roster.
- **Identity as a resolver seam** — `USER_ID = "localUser"` (`getCurrentUserId()`, `src/context/userContext.ts`) wants the treatment `activePack` got: resolvers behind a seam module (`currentUser()`, `usersRoster()`) rather than a constant read at import time, with fake users as a fixture beside `demoTree` in `src/data/fixtures/` so multi-user UI can be built and looked at before any auth exists. The pack seam is the worked example — the shape is known, only the decision to start is outstanding. This is the first step of a Users branch, and the thing ISSUES → Architecture *The cascade / arbiter* is waiting on.
...
- **Editing a picked Definition's config from the Add Surface** — the row shows a picked Definition's config read-only, and its name goes `readOnly` too, because rewriting either would change shared meaning from a surface whose job is to add. Under fork-never-mutate the shape this wants is a **link plus a fork**. **The link half landed 2026-08-22**: the Config band's provenance line (*from Weight*) is the affordance, and because `ConfigSummary` is the same component on both callers, a persisted Field's Config band got it in the same change. What remains is the **fork** — an actual change minting a successor Definition and rebinding the instance, which is the undecided affordance ISSUES → *Decide Field re-configure UX* holds. Until it exists the escape hatch is Cancel and retype. Raised 2026-08-15; reframed 2026-08-20 (the 2026-08-17 edit-in-place reframing died with the place-design).
