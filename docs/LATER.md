...
- **A failed fetch must not degrade silently.** `provisionLenses.ts:41-49` stamps lens policy only if resolvable; with a fetched pack, that guard becomes the path a fetch failure takes, minting unbound lenses — and nothing re-stamps them afterwards, since the backfill only creates lenses it finds missing. Bundled fallback; never boot packless — and note the seed half of *packless* is already reachable today, because `doInitializeStorage` swallows its own failure and still reports initialized (both filed in ISSUES → Bugs).
