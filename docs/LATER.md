...
- **Netlify deploy is real but unconfigured.** The app has deployed to `complete-maintenance-management.netlify.app` on every merge to `master` for months — the Phase V note claiming "no hosting target" was wrong, written from the repo's Firebase config rather than the actual pipeline. `netlify.toml` is still the scaffold Netlify generated: build command + publish dir, with the SPA redirect commented out and no header rules. Two things it needs when the deploy is next touched — the `/*` → `/index.html` 200 rewrite (harmless today only because the FSM never puts state in the URL, so no deep link exists to 404), and `Cache-Control: no-cache` on `/service-worker.js` so a new build's SW is actually fetched instead of served from the CDN edge.
