...
- ~~**Standalone-row UX for the config-only kinds**~~ — **no longer deferred.** `flag` and `string-list` need real editable renderers (a toggle, a chips list) the moment the Library surfaces a Definition's config as editable rows, which is the current work; `compound` is retired (SPEC → *Config is Elements*). Tracked in ISSUES. What *stays* deferred is offering these two kinds in the Add Surface picker — see *Config-only kinds excluded from the picker* below.
- **Disposition honoring (cascade arbiter)** — `owned`/`delegated`/`pinned` is **encoded** on each `ConfigSubField` but nothing acts on it; everything reads live from the Definition. Copy-at-mint for `owned` and override-disable for `pinned` are the cascade arbiter's job (ISSUES Architecture #4). **The safety net this note used to name is gone**: edit-is-fork no longer prevents a Definition change from reaching existing instances, because a Definition is now mutated in place and propagates downstream by design (SPEC → *Edit / Delete Semantics*). Disposition is therefore the *only* planned brake on a Library edit arriving silently everywhere, which raises its priority from "arbiter tidy-up" to "the first real intervention on propagation".
- **Per-sub-field reactive signals** — renderers assemble the whole config object on read via `getDefinitionById` keyed on `definitionId`. Live propagation of an individual Definition sub-field edit into mounted instances is **now the intended behaviour**, not an unneeded one, so what is deferred is only the granularity: whole-config reassembly on a Definition write is correct and probably fast enough; per-sub-field signals are an optimisation to reach for if it isn't.
...
- **Config Fields as instances** — a Definition's config Fields carry `definitionId: null` today. The end state is that they are minted from seeded meta-Definitions ("Units symbol", "Decimals", "Threshold LL"), so *every* Field on *every* card in the app, including inside the Library, is an instance of something and the last `null` case disappears. Strictly additive over what ships now — the identity test (`definitionId === id`) does not change, only that config Fields stop being `null` — so it is a seed set plus a backfill, not a rework. Deferred because it makes the Library visibly contain its own scaffolding, which is a UX conversation of its own. Decided as the destination 2026-08-17.
...
- ~~**Grouping the Library by pack provenance hits the same blocker as categories.**~~ **Unblocked 2026-08-17.** The export sketch groups the Library into "From: <pack>" and "Authored here", which needs a Definition to sit *under* something; that was blocked while `parentId === null` was the storage layer's identity test. Identity moved to `definitionId === id`, which is position-independent, so both this and arbitrary categories are now free. (The reconciliation the *Sections* note below asks for resolves the same way: pack provenance is a tag, so grouping by it was always a view and blocked by nothing.) Noticed 2026-08-14; resolved by the Library-as-a-place work.
...
- **`componentVersion` field** on FieldDefinition (per-FieldComponent contract versioning) — only relevant once FieldComponent config schemas evolve, and less likely than it was: a reconciling config ProvisionSpec grows existing Definitions into a widened schema without a version to migrate between (SPEC → *Config Fields are provisioned*).
- **Ownership rules for edit & delete** ("you can delete / edit your own"). **Edit itself is no longer deferred** — a Definition is edited in place in the Library and propagates downstream (SPEC → *Edit / Delete Semantics*, 2026-08-17). What waits on identity is *who may*, plus non-deletability as a general Element property rather than an affordance withheld from the Library Node. Delete stays admin-only via direct Firestore writes.
- **Notification / gating between a Library edit and its arrival everywhere** — an edit reaches every client on next sync, silently, and changes what every bound instance means. Wiring `disposition` is the first brake (see *Config-as-Elements → Disposition honoring*); beyond it sit change summaries, "N instances affected" before the write lands, and any manager gate. Upstream propagation is **not** on this list: it is rejected outright, not deferred.
...
- **Library discovery UX**: popularity ranking, "recently added" sort. **Grouping by kind is not deferred** — it ships as a view (the Kind band), needing no re-parenting. *Arbitrary user-authored groups* are **no longer blocked either**: the identity test moved off `parentId === null` to `definitionId === id` (2026-08-17), which is position-independent, so a Definition may sit under a folder Node inside the Library without ceasing to be one. What remains deferred is the grouping UI — the folder kind, the drop targets, the empty-group rule. Restated twice: 2026-08-15 (all grouping is not blocked) and 2026-08-17 (nor is any of it, structurally).
- **Editing a picked Definition's config from the Add Surface** — the row shows a picked Definition's config read-only, and its name goes `readOnly` too, because rewriting either would change shared meaning from a surface whose job is to add. **Edit-as-fork is no longer the answer**: a Definition is now mutated in place, and the place is the Library (SPEC → *Edit / Delete Semantics*, 2026-08-17). So the shape this wants is a **link, not an edit** — tap through from the picked Definition to its Node in the Library, change it there, come back. That is the same affordance ISSUES → Features wants from a persisted Field's Config band, and it is one affordance, not two. Until it exists the escape hatch is still Cancel and retype. Raised 2026-08-15; reframed 2026-08-17.
...
- ~~**Dedicated Library view**~~ — **no longer deferred, and not a "view".** Specced 2026-08-17 as SPEC → *The Library*: a Library Node pinned at ROOT, Definitions as its child Nodes, config as Fields on each Definition's card. Not a stack under a main menu and not a second view layer — one tree, ordinary re-rooting. It is still the only place a Definition becomes editable; the Add Surface only ever adds.
