...
- **Library discovery UX**: popularity ranking, "recently added" sort. **Grouping by kind is not deferred** — it ships as a view over the `library` tree (the Kind band), needing no re-parenting. Only *arbitrary user-authored groups* stay blocked: `parentId === null` is the storage layer's test for "is a Definition" (`IDBAdapter.listDefinitions`), so a Definition cannot sit under a category node until that identity test moves. Restated 2026-08-15 — the earlier entry called all grouping blocked, which was wrong.
- **Editing a picked Definition's config from the Add Surface** — the row shows a picked Definition's config read-only because config is `delegated` and rewriting it would change shared meaning. Edit-as-fork (touch the config and the row silently becomes a new draft Definition, name and config pre-filled) is the obvious shape and would also answer "Decide Field re-configure UX"; deferred deliberately so the surface lands with one commit path. Raised 2026-08-15 while speccing the Add Surface. The build made the same call for the *name*: it goes `readOnly` while a Definition is picked, because the mint snapshots `definition.label` and an edit there would be a lie. Edit-as-fork is what un-freezes both at once — until then the escape hatch is Cancel and retype.
