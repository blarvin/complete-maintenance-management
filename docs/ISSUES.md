...
14.) **jcodemunch index refreshes only on commit** — the PostToolUse reindex hook did not pick up Edit-tool writes to `src/styles/global.css` or `docs/ISSUES.md`, nor a `git rm`; all three reported as "uncommitted change(s) … had not reached the index" until committed, after which symbol search was correct. `get_watch_status` shows `service: { platform: 'schtasks', active: false }` — the watch-all daemon isn't running on this machine. Separately, `search_text` finds nothing for strings that exist in `.css` / `.md` while matching `.ts` fine, so a jcm "not found" on styles or docs is not evidence of absence. Observed 2026-08-10 across three queries during the app-width fix.
