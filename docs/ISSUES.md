...
13.) **A failed Cypress run leaves `cypress/screenshots/` untracked in the working tree** — it is not in `.gitignore`, so a red spec turns into an untracked directory that shows up in the next `git status` and is easy to stage by accident. Surfaced running the seed-coupled specs for Field-Packs, 2026-08-21.
