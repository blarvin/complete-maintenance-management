...
11.) **DEVELOPING.md's wipe table omits `npm test`** — `firestoreAdapter.test.ts` calls `wipeEmulator()` (DELETE across the whole project) in `beforeEach`, and it sits in the default run behind `describe.skipIf(!emulatorUp)`, so a plain `npm run test` clears the emulator whenever the emulator happens to be up. Accepted behaviour (Cypress already wipes it every spec, and the mocked `../data/firebase` means the suite has no path to production) — it just isn't in the wipe table at DEVELOPING.md §"which reset to reach for", which lists `__wipeLocal` / `wipe:emulator` / `__wipeDefinitions` and documents the Cypress wipe separately. Add it as the fourth. Observed 2026-08-13: ran `npm test` during the Tech Debt #4 work with the emulator up and cleared it.

12.) **Emulator coverage is adapter-level only, never sync-level** — `firestoreAdapter.test.ts` (2026-08-12, the spec that caught the delta-sync Timestamp/number mismatch) round-trips `applySyncItem` and the pulls against a real emulator, so the adapter lane is covered. Nothing exercises `SyncManager` against a real server: a full push→pull cycle, server-authority resolution with a genuinely pending local edit, cursor advance across cycles. Both 2026-08-13 fixes land exactly there and are mock-only — the high-water cursor (`syncCursor.test.ts`) and history convergence (`historyConvergence.test.ts`, where the real proof is two clients' appends surviving a round trip through Firestore, not through two `db.elementHistory.put` calls). History has one emulator test; two-client convergence has none. Carried over from LATER §"Emulator Round-Trip Sync Coverage", deleted 2026-08-13 — its premise (no emulator suite exists, needs a Node connect path + separate vitest config) was overtaken by the `vi.mock('../data/firebase')` approach, and its third target, `fieldDefinitions`, stopped existing at schema v10.