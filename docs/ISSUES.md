...
9.) **Config-only kinds still share the stub renderer, and the Add Surface spec surfaces them** — `flag`/`compound`/`string-list` all render through `ConfigFieldStubRenderer` (`configFieldStub.ts`), a bare text thunk: `—` for null, `a, b, c` for a list, `JSON.stringify` for an object. LATER deferred real renderers "if/when config sub-fields are ever surfaced as editable rows"; the new spec surfaces them in two places (the picker's config peek, Field Details → Config), so that condition is met. Read-only is survivable for `flag` and `string-list`; a `number-kv` threshold compound rendering as `{"LL":0,"L":10,...}` in front of a user is not. Minimum three small read-only renderers; only `flag` needs an editable one, since `number-kv` and `enum-kv` keep their ConfigForms. Blocks the Add Surface build. Observed 2026-08-14.

10.) **Snackbar has no coalescing, and the Add Surface spec assumes it** — SPECIFICATION.md → Snackbar & Undo now specs a repeated same-kind action extending the current toast into one reversal closure ("3 fields added"), because single-slot `show()` replacement would leave only the last of several picks undoable. `snackbarService` today does the opposite: `show()` runs the prior toast's `onExpire`, cancels its timer, and replaces it. Needed before multi-pick is honest. Observed 2026-08-14 against SPEC §175/§230.
