...
8.) **`mintVia: 'composer'` outlives the composer** — the SPEC now names the value `'add-surface'` (§manifest sketch); the code still says `'composer'` in `mintVia.ts`, `types.ts` and the `FIELD_KINDS` filter in `registry.ts`. Three files, mechanical, and exactly the `cardOrder` situation in Tech Debt #4 if left. Rides the Add Surface build. Surfaced 2026-08-14 while speccing it.

9.) **Snackbar has no coalescing, and the Add Surface spec assumes it** — SPECIFICATION.md → Snackbar & Undo now specs a repeated same-kind action extending the current toast into one reversal closure ("3 fields added"), because single-slot `show()` replacement would leave only the last of several picks undoable. `snackbarService` today does the opposite: `show()` runs the prior toast's `onExpire`, cancels its timer, and replaces it. Needed before multi-pick is honest. Observed 2026-08-14 against SPEC §175/§230.

10.) **Retire the composer stack — last, not first** — `FieldComposer/`, `CreateDataField/`, `usePendingForms`, the `pendingFields:` localStorage, `pendingMode` across the four kv renderers, and the roster itself all go when the Add Surface wins. Build the Add Surface as a **third roster id** first (`addFieldSurfaces.ts` already documents the four steps, and the mutex means only one opens at a time), so all three surfaces can be compared in the running app by editing `ENABLED_ADD_FIELD_SURFACES` — the comparison the roster was built for, and worth more than reading the old code. Consequences of that ordering: the composer needs `pendingMode` to keep working, so the renderer cleanup joins the deletion commit rather than preceding it; and `useFieldEdit`'s pending lifecycle (Tech Debt #2) resolves there too. **Tag the parent commit at deletion** so the composer is reachable by name rather than by hash. Decided 2026-08-14.
