...
19.) **The Add Surface's value slot and a Definition's `default` disagree about the preview** — `CREATE_ELEMENT_FROM_DEFINITION` now falls back to `def.config.default` when no `initialValue` is supplied, so picking a defaulted Definition mints a filled field. But the draft row's value slot still shows `Empty` while it does so, which breaks *the row is the Field* (SPEC → The Add Surface → Preview fidelity). Two candidate fixes, both small, neither obviously right: seed `draft.value` from the picked Definition's default (honest preview, but it overwrites a value the user may have typed before picking), or leave the preview and accept that a defaulted pick fills in on commit. Related wart from the same `??`: clearing the value slot on a defaulted Definition still mints the default, because `null` cannot be told from *untouched*. Observed 2026-08-16 wiring the default through to mint.
