...
> **✅ Done** (2026-06-27, commit 16c58ed — IMPLEMENTATION.md → *Config-as-Elements*).
...
- **✅ Value-shape vocabulary (done 2026-07-05):** `ValueShape = 'scalar' | 'block' | 'composite'` required on `ValueSpec.shape`, authored in `capabilities.ts`; the arrangement laws live in the one `DataField` dispatcher (no `ownValue` → scalar default, covering `asset-doc`); `hideLabel`/`blockValueLayout` retired; `stream` joins with its first consumer (ISSUES). Ride-alongs done: ISSUES #9 — `renderMode.ts` (`nodeRenderMode(kind)` collapses the triplicated lens/plain/chip match); ISSUES #16 — `KindValueMap` in `models.ts` derives `DataFieldValue` from the registry. (IMPLEMENTATION.md → *#5 value-shape*.)
...
**Sequencing instinct:** 1 → 2 → 3 are the spine and unlock everything (done; #2's capability seam landed 2026-06-28); 4's seam is in. **#6a/#6b are built** (2026-06-28, stub-grade) and **#5 slices 1–2 + the container half shipped** (2026-06-30) — the both-rollup-and-container shape was prototyped on `jobs` (a hybrid container; jobs authored inside, node-owned). `logbook`**/**`log-entry` **(the lens's second instance) landed 2026-07-01** (minimal scope — lens generalization + spec-driven provisioner), and **the #7a–c Definition-binding seam landed the same day** (full `definitionId` type-family rename + placement-agnostic authoring contract + logbook's seeded policy Definition — #7's remaining core is the arbiter itself). **The §5 value-shape vocabulary + the ISSUES #9/#16 registry consolidations landed 2026-07-05** (`ownValue.shape` + the one `DataField` law table; `renderMode.ts`; `KindValueMap`) — closing this branch's registry/manifest charter. **Next:** the fuller **#6c catalogue** (the `Edges` family / `asset-gallery` / `person` / `logical-container`), or the #7 arbiter — each its own plan on its own branch. 6/7 are the big new-capability build and the natural place to slice into per-kind plans; 8 falls out of 3/6. Open design questions (ProvisionSpec triggers, aggregation materialization, person/org sub-specs, arbiter classification) are flagged in ELEMENT-MODEL.md and worth resolving before building 6/7.

Caveat for the unbuilt clusters (6c onward): file names/seams (`src/kinds/`, the `ConfigForm`s, `usePendingForms`) are framed as the docs describe them — confirm against the current tree when writing the real plan.
