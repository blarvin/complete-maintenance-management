...
- **Decouple placement's two axes.** Today `placement: inline | re-root` conflates *render-location* (Data-Card row vs re-rooted view) with *navigability*. The `jobs`-as-container ask — each `job` an inline Data-Card row that is *still* re-rootable — needs render-location driven by the **container's `childrenSpec`**, independent of whether the child is navigable. (The create-surface half of this — restrict/hide by `childrenSpec` — shipped in slice 1; the render-location half + navigable rollup rows wait on `jobs`-as-container, #6c.)
