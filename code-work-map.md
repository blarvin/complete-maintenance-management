...
1. #6c Edges family / #7 arbiter â these are the big remaining new-capability builds, and I'd not start them on this branch. Both are blocked on things the map itself flags: the ancestors/edges traversals still throw in capabilityEngine.ts (ISSUES #11), and [ELEMENT-MODEL.md](http://ELEMENT-MODEL.md) has open design questions (arbiter classification, ValiditySpec, person/org sub-specs) the map says to resolve before building. Each is its own plan on its own branch.
2. Everything else is blocked or deferred: rich lens rows wait on Action (built last, per the map), per-viewer overlays wait on auth + the arbiter, and #8's image rework falls out of the sub-field seam whenever you want it.
