...
## 5. Chrome entailment
> **◐ Slice 1 done** (2026-06-29): the create surface + shell read `childrenSpec` — `childrenPolicy.ts` (`allowedChildKinds`/`canHaveChildren`) + `reRootCreateKindsFor` (registry); the node-create picker offers only admitted kinds (`job`→node/job, lens→none) and the lens-aware shell drops the DataCard/chevron for content-free kinds (IMPLEMENTATION.md → *#5 slice 1*). The first consumer of `childrenSpec.allowedKinds`. **Remaining:** the render-location decoupling + the value-shape vocabulary below.
...
- **Decouple placement's two axes.** Today `placement: inline | re-root` conflates *render-location* (Data-Card row vs re-rooted view) with *navigability*. The `jobs`-as-container ask — each `job` an inline Data-Card row that is *still* re-rootable — needs render-location driven by the **container's `childrenSpec`**, independent of whether the child is navigable. (The create-surface half — restrict/hide by `childrenSpec` — shipped in slice 1; the render-location half is **slice 2**, below.)
- **Slice 2 (next) — jobs-as-navigable-field-rows: the render-location decoupling's first real consumer.** Today the lens rollup (`KindAdornment`, isParent) lists job names as dead text. Render each gathered job as a compact **field-styled, navigable, expandable row**: the name re-roots (`navigateToNode$(job.id)` — identical to clicking its CHILD card), and a details chevron (reusing the `DataCard`/`DataFieldDetails` expand pattern) opens the job's own `FieldList` — its sub-fields/config — inline. This is `render-location: inline + navigability: yes` made concrete, and it's almost pure recomposition: reuse `navigateToNode$`, `FieldList`, and the field-row look + details-expand affordance (NOT the `DataField` component itself — it's welded to value-editing; borrow the skin, not the guts). **Build it as a generic *navigable inline row* a container/lens hands its children**, not a jobs special-case, so `logbook` (#6c) inherits the same row — that genericity is the whole reason this was parked behind "wait for a second container," and it's gotten without waiting. This is the slice that crosses a seam end-to-end and turns the lens from a stub into a usable surface. Open question deferred: a job row's *primary* line/history (likely the status/lifecycle line once `Action` lands) — start with name + FieldList-on-expand.
